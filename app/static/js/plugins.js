...
function display_message(user, time, message, privateMessage) {
    ...
    if (!privateMessage && user.id === self_user.id && self_user.user_turns) {
        self_user.current_turn = false;
		// controlUserTurn(user.id);

        // if (user.id === self_user.id && isWizard()) {
        //     wizardLastUtterance = message;
        // }
	}

    // reload dialogue options if this user receives a message from the other user, but this one is the wizard
    if (user.id !== self_user.id && isWizard() && !isUserBot(user.id) && !self_user.user_turns) {
...
function display_image(user, time, url, width, height, privateMessage) {
	...
	// scroll the chat down once the image has loaded
	text.find("img").one("load", function() {
		_scrollChat();
	})
    _append(text);
}
